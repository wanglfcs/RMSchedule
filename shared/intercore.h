...
// Place "addr" on WQ, without any matching data.
// Useful assembly code for some of the I/O device operations
...
int sfb_internal(int core, void *frame);

static int setFrameBuffer(void *frame) {
  // Tell the display controller the location of its current frame buffer.
  // Blocks until the controller responds.
  // Assumes that "frame" is cache line aligned.
  //
  return sfb_internal(enetCorenum() + 1, frame);
}
