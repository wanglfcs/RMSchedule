...
void *malloc(size_t size) {
  // malloc1 allocates items in spearate cache lines.  But additionally,
  // we have to put malloc's 3-word header in a separate cache line, because
  // it gets updated on core #1 and data updates in other cores could
  // overwrite it incorrectly.  (Thanks, Nikolai.)
  //
  if (corenum() == 1) {
    return malloc1(size + 32 - 12) + (32 - 12);
  } else {
    clearResponse();
    IntercoreMessage msg;
    msg[0] = mcMalloc;
